...
prepare_dataset <- function(activity, features, type) {
        ...
        # Join the datataset by 'activityId' is performed. Note: Join method does not mix the row order like merge... 
        ds_lbl <- join(ds_lbl, activity)
        ...
        ds_values <- select(ds_values, 1:6, 41:46, 81:86, 121:126, 161:166, 201:202, 214:215, 227:228, 240:241, 253:254, 266:271, 345:350, 424:429, 503:504, 516:517, 529:530, 542:543)
        
        # Build dataset as a Tidy dataset
        ds <- cbind(ds_sub, ds_lbl, ds_values)
